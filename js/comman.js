...
$(document).ready(function () {
    ...
    var swiperProduction = new Swiper('.production-tab-content .swiper-featured', {
        loop: true,
        navigation: {
            nextEl: '.swiper-button-next',
            prevEl: '.swiper-button-prev'
    ...
    $('#auth_purchaser').on('change', function () {
        $(this).is(':checked') ? $('#auth_purchaser_block').show() : $('#auth_purchaser_block').hide();
    }).trigger('change');

    $('#not_vat_company').on('change', function () {
...
try {
    let linkActivation = new ActiveLinks('.production-sticky-tabs .tab', '.production-tabs-content-wrapper');
} catch (e) {
    console.log(e)
}
...
function initMap() {
    map = new google.maps.Map(
        document.getElementById('map'), {
            center: new google.maps.LatLng(56.9170740, 23.9907740),
            zoom: 14
        });

    var iconBase =
        'http://aleksagw.beget.tech/web/images/icons/';
    ...
    var features = [{
        position: new google.maps.LatLng(56.9170740, 23.9907740),
        type: 'pin',
    }, {
...
let plusOrMinus = document.querySelectorAll('.product-quantity span')
console.log(plusOrMinus)
for (let pls of plusOrMinus) {
    pls.addEventListener('click', function (e) {
        let currentAttr = this.getAttribute('name'),
            parent = this.closest('.product-quantity'),
            inputA = parent.querySelector('#order_qty'),
            valueAttr = inputA.getAttribute('value');
        if (currentAttr == 'inc' && inputA.value < 999) {
            inputA.value++
        } else if (currentAttr == 'dec' && inputA.value > 0) {
            inputA.value--
...
$(function () {
     ...
     */
    var input = document.querySelectorAll("input[name=country-mask-phone]");
    ...
    for (var i = 0; i < input.length; i++) {
        ...
        $('input[name="country-mask-phone"]').on("focus click countrychange", function (e, countryData) {
        ...
        $('input[name="country-mask-phone"]').on("focusout", function (e, countryData) {
